REM ff14-download-timing-script
...
color 4F
color 01
SET ACTUALGAMENAME=ff14-download-timing-script
...
cls
color 01 & cls & goto begingame & cls & color 01
color 01
cls
goto begingame
cls
...
color 01
cls
cd ff14-download-timing-script
cls
...
REM skip branding now
set BRAND={purple}{default}
set BRAND2={teal}{default}
set BRAND3=%blockred%
...
CALL .\depends\cecho  {black}.......................{white}{default}{\n}
CALL .\depends\cecho  {black}
CALL .\depends\cecho  {white}
...
if %roomid%==3 set ROOMDES1=.

if %roomid%==3 set ROOMDES2=.
...
time /t
echo -----------
echo I will start FF 14's launcher at: 05:00 AM (when the internet is available)
echo.
echo Disabling UAC (User Account Control) may be required to bypass a manual action.
echo I cannot automate pressing the Yes button for you, for obvious reasons.
echo.
echo Waiting
...
REM /GET TIME
REM echo tfa is o%timeforalarm%o

REM process time so it's usable
set  process=%timeforalarm%
REM echo processed to be readable: a%timeforalarm:~0,-9%a
REM or to cut it in the variable:
set timeforalarmfixed=%timeforalarm:~0,-9%

set timeforalarmfinal="%timeforalarmfixed%"
...
if %timeforalarmfinal%==" 5" goto foxartdevtest
REM /act at alarm time

goto updateroom

REM ENTIRE ALARM CLOCK LOGIC END
...
if %MENUINIT%==0 goto foxartdevtest
if %MENUINIT%==1 goto newmainmenuresume
...
:gobacktoforest
if %roomid%==1 goto errornotroom
if %roomid%==99 goto errornotroom
if NOT %roomid%==3 goto errornotroom
set renderweightdisplay=1
CALL .\depends\cecho  {yellow}You move back to the earlier forest area.{default}
 echo.
echo Please type intro to begin.

CALL .\depends\cecho But yes, that is a command you can use whenever you wish to look around the     current area.
...
set FIRST==0
cls
echo It's 5am, I'm starting the launcher so your game will download, now.
echo.
echo Working...
"C:\Program Files (x86)\Steam\Steam.exe" steam://rungameid/312060
timeout /t 20 >NUL
"switch-to-ff-launcher.ahk"
timeout /t 2 >NUL
"input-pw-into-launcher.ahk"
timeout /t 2 >NUL
echo It should be downloading, now, enjoy!
echo.
...
:newintro
if %roomid%==99 goto indeeditis
if NOT %roomid%==1 goto errornotroom
set ROOMID==99
cls
echo ? intro
CALL .\depends\cecho  {white}Welcome to %ACTUALGAMENAME%- a game where you play as a cute lovely fox,{\n}{default}

echo I hope that you have a nice time on a fun little day out, and enjoy yourself.
CALL .\depends\cecho  I'm sure you'll go to some lovely places and meet some interesting characters~
echo.
...
echo This game uses regular text adventure commands, a lot, but also relies on typing the words that you see as [highlighted phrases], like that one.
CALL .\depends\cecho  When you're ready, type [start] to begin playing.
 echo.
CALL .\depends\cecho  This game doesn't have saving, and as such, it's a short one-shot. Enjoy~{\n}

goto updateroom


:thatsright
if NOT %roomid%==99 goto errornotroom
echo Yes, exactly like that~
goto updateroom
